#!/usr/bin/env node
/**
 * Setup wizard for Devonz local installs.
 *
 * Goals:
...
function isSensitiveKey(key) {
  ...
  return k.includes('KEY') || k.includes('TOKEN') || k.includes('SECRET') || k.includes('PASSWORD') || k.includes('AUTH');
}
...
function questionHidden(rl, prompt) {
  return new Promise((resolve) => {
    ...
    // Try the private _writeToOutput hook for input masking.
    // If it's unavailable in a future Node version, fall back to normal prompts.
    if (typeof rlAny._writeToOutput !== 'function') {
      rl.question(`${prompt}(visible) `, resolve);
      return;
    }
...
async function interactiveWizard() {
  ...
  try {
    ...
    while (true) {
      const pick = (await question(rl, 'Enter comma-separated env keys to set (blank to finish, "list" to show all): ')).trim();

      if (!pick) {
        break;
      }

      if (pick.toLowerCase() === 'list') {
        const sorted = [...knownKeys].sort((a, b) => a.localeCompare(b));
        process.stdout.write(sorted.join('\n') + '\n');
        continue;
      }

      // Preserve original case — don't force uppercase so keys like HuggingFace_API_KEY work.
      const keys = pick
        .split(',')
        .map((x) => x.trim())
        .filter(Boolean);
      ...
      for (const key of keys) {
        if (!knownSet.has(key)) {
          // Try case-insensitive match before warning.
          const ciMatch = knownKeys.find((k) => k.toLowerCase() === key.toLowerCase());

          if (ciMatch) {
            process.stdout.write(`Using "${ciMatch}" (matched from .env.example)\n`);
            await promptAndSetKey(rl, parsed, ciMatch, updatedKeys);
            continue;
          }

          const cont = (await question(rl, `Key "${key}" is not in .env.example. Set anyway? (y/N) `)).trim().toLowerCase();
        ...
        await promptAndSetKey(rl, parsed, key, updatedKeys);
      }
...
async function promptAndSetKey(rl, parsed, key, updatedKeys) {
  ...
  const valueRaw = sensitive ? await questionHidden(rl, prompt) : await question(rl, prompt);
